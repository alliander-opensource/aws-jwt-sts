...
- **`@types/node`** stays on `^24`, matching the `node-version` both workflows pin. These typings are the upper bound on what the handlers in `src/index.sign.ts` and `src/index.keyrotate.ts` can reference and still compile, and those handlers deploy to `Runtime.NODEJS_22_X`, so the bound should not run far ahead of the runtime. Concretely, `@types/node@26` declares `node:quic`, `node:ffi` and `node:vfs`, none of which exist on a Node 22 Lambda; `@types/node@24` declares the same module set as `@types/node@22`, so it does not reintroduce that class of mistake. Do not raise this to `^26` or later without moving the Lambda runtime first.
